function [PY,OutputWeight,H,M,Xps,Yps]=F_OSELMtrain_Add1(X,Y,ActivationFunction,Similarity ,Elm_Type,beta,X1,InputWeight,BiasMatrix,k,M1)
%%%%%%%%%%%%% Macro definition
 REGRESSION=0;
 CLASSIFICATION=1;
%% 归一化
...
switch lower(ActivationFunction)
     ...
     %% 不遗忘数据
      %M是工作矩阵（过渡的） 
      M =(M1 - M1 * H' * (eye(size(H,1)) + H * M1 * H')^(-1) * H * M1); 
      OutputWeight=beta + (M * H' * (T' -H* beta));  
